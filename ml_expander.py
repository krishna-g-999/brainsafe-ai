...
try:
    ...
    _RDKIT_OK = False
    log.warning('RDKit unavailable — Morgan FP disabled')
import requests
...
# ── Morgan fingerprint support (module-level) ─────────────────────────
import pathlib as _pl
_SMILES_CACHE_PATH = _pl.Path(__file__).parent / 'smiles_cache.json'
_SMILES_CACHE: dict = json.load(open(_SMILES_CACHE_PATH)) \
    if _SMILES_CACHE_PATH.exists() else {}
FP_BITS = 64
try:
    from rdkit import Chem as _Chem
    from rdkit.Chem import AllChem as _AllChem
...
def _morgan_fp(smiles: str) -> list:
    ...
    try:
        ...
        if mol is None:
            return [0.0] * FP_BITS
        fp = _AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=FP_BITS)
        return list(fp)
    except Exception:
        return [0.0] * FP_BITS
# ── End Morgan FP support ──────────────────────────────────────────────
...
def build_training_data(
    ...
    for name, entry in curated.items():
        ...
        # Extend feats with 64-bit Morgan fingerprint
        _smiles = _SMILES_CACHE.get(name, '')
        feats.extend(_morgan_fp(_smiles))
...
def train_ml_model(
    ...
    cv_scores = cross_val_score(
        MultiOutputRegressor(RandomForestRegressor(n_estimators=100, max_depth=5, random_state=42)),
...
def run_expansion(
    ...
    def _nps(pred: dict) -> float:
        NPS_W = {
            ...
            "mitochondrial_support": 2, "aggregation_modulation": 2,
            "cognitive_enhancement": 1, "neurogenesis": 1,
            "synaptic_plasticity": 1,
        }
        NPS_MAX = 130
        raw = sum(pred.get(col, 5.0) * w for col, w in NPS_W.items())
        return round(min(100.0, raw / NPS_MAX * 100.0), 1)
    ...
    def _mol_metadata(mol: dict) -> dict:
        ...
        return {
            ...
            "feat": [float(bbb_num),
                     ...
                     # Morgan fingerprint from ChEMBL canonical_smiles
                     *_morgan_fp((mol.get('molecule_structures') or {}).get('canonical_smiles', '')),
                     3.0],
    ...
    ml_compounds["_ml_metadata"] = {
        ...
        "model":              "MultiOutputRandomForestRegressor (n_estimators=150, disease-feature basis)",
