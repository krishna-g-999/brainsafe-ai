...
"""
================================================================================
ULTIMATE HtrA ANALYSIS - FULLY AUTOMATED (WINDOWS UTF-8 FIXED)
================================================================================

✅ FIXED: Windows encoding issue with Ų symbol
...
USAGE:
    python ULTIMATE_HTRA_ANALYSIS_FIXED.py
...
print("="*80)
print("ULTIMATE HtrA ANALYSIS - FULLY AUTOMATED (UTF-8 FIXED)")
print("="*80)
...
def calculate_sasa_for_all(pdb_files, distance_results):
    ...
    for result in distance_results:
        ...
        try:
            ...
            print(f"  ✓ {pdb_id}: Avg SASA = {avg_sasa:.1f} A² (His={his_sasa:.1f}, Asp={asp_sasa:.1f}, Ser={ser_sasa:.1f})")
...
def prepare_pqr_files(pdb_files):
    ...
    residue_charges = {
        ...
        'ALA': 0.0, 'VAL': 0.0, 'LEU': 0.0, 'ILE': 0.0, 'MET': 0.0,
    ...
    for pdb_id, pdb_file in pdb_files.items():
        ...
        with open(pdb_file, 'r', encoding='utf-8') as f_in, \
             open(pqr_file, 'w', encoding='utf-8') as f_out:
            for line in f_in:
                if line.startswith('ATOM'):
    ...
    with open(instructions_file, 'w', encoding='utf-8') as f:
        ...
        f.write("="*80 + "\n\n")
        f.write("Your PQR files are ready!\n\n")
        f.write("Go to: https://www.poissonboltzmann.org/apbs/\n")
        f.write("Upload each .pqr file and generate electrostatic maps\n\n")
...
def generate_pymol_script():
    ...
    with open(script_file, 'w', encoding='utf-8') as f:
        f.write("# PyMOL Figure Generation\n\n")
        ...
        for pdb_id, config in Config.PDB_IDS.items():
            f.write(f"# {pdb_id}\n")
            f.write(f"fetch {pdb_id}\n")
            f.write(f"hide everything, {pdb_id}\n")
            f.write(f"show cartoon, {pdb_id}\n")
            f.write(f"color grey70, {pdb_id}\n")
            f.write(f"select triad, {pdb_id} and chain {config['chain']} and resi {config['his']}+{config['asp']}+{config['ser']}\n")
            f.write(f"show sticks, triad\n")
            f.write(f"util.cbaw triad\n")
            f.write(f"zoom triad, 8\n")
            
            output_file = os.path.join(Config.FIGURES_DIR, f"{pdb_id}_triad.png")
...
def run_pymol_figures():
    ...
    print("  ⚠ Run manually: Open PyMOL → Run generate_figures.pml\n")
    return False

# ============================================================================
# OUTPUT FILES (UTF-8 FIXED)
# ============================================================================
...
def save_distance_csv(results):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("PDB_ID,Protein,Residues,His_Asp_Distance_(A),His_Ser_Distance_(A),Asp_Ser_Distance_(A),Catalytic_State,Activity\n")
        for r in results:
...
def save_sasa_csv(sasa_results):
    """Save SASA results - UTF-8 FIXED"""
    output_file = os.path.join(Config.SASA_DIR, 'sasa_values.csv')
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("PDB_ID,Protein,His_SASA_(A²),Asp_SASA_(A²),Ser_SASA_(A²),Avg_SASA_(A²),Catalytic_State\n")
...
def save_comprehensive_report(distance_results, sasa_results):
    """Save master report - UTF-8 FIXED"""
    output_file = os.path.join(Config.BASE_DIR, 'ULTIMATE_ANALYSIS_REPORT.csv')
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("PDB_ID,Protein,Organism,Description,His_Asp_(A),His_Ser_(A),Asp_Ser_(A),Avg_SASA_(A²),State,Activity,Interpretation\n")
        ...
        for i, d_res in enumerate(distance_results):
            ...
            if d_res['state'] == 'COMPETENT':
                interp = "Active protease with optimal catalytic geometry"
            elif d_res['state'] == 'DISTORTED':
                interp = "Inactive conformation - regulatory OFF state"
            elif d_res['state'] == 'ALLOSTERICALLY_LOCKED':
                interp = "Structurally locked - scaffolding function"
            else:
                interp = "pH-dependent - requires specific conditions"
...
def create_publication_table(distance_results, sasa_results):
    """Create publication table - UTF-8 FIXED"""
    output_file = os.path.join(Config.BASE_DIR, 'PUBLICATION_TABLE.txt')
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*120 + "\n")
        f.write("PUBLICATION TABLE: HtrA Catalytic Triad Analysis\n")
        f.write("="*120 + "\n\n")
        
        f.write(f"{'PDB':<6} {'Protein':<18} {'His-Asp (Å)':<12} {'His-Ser (Å)':<12} {'SASA (A²)':<12} {'State':<20} {'Activity':<12}\n")
        f.write("-"*120 + "\n")
...
def main():
    ...
    print("✅ COMPLETE!")
    print("="*80 + "\n")
    print(f"Results: {Config.BASE_DIR}/\n")
    print("Main files:")
    print("  ✓ ULTIMATE_ANALYSIS_REPORT.csv")
    print("  ✓ PUBLICATION_TABLE.txt")
    print("  ✓ catalytic_triad_distances.csv")
    print("  ✓ sasa_values.csv")
    print("  ✓ electrostatics/*.pqr\n")
    print("="*80 + "\n")
