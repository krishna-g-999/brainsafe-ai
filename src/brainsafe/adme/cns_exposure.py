...
def _load():
    ...
    for ep in ("caco2_permeability", "lipophilicity", "solubility",
               "plasma_protein_binding", "pgp_inhibition"):
...
def predict(smiles, m):
    ...
    favourable = (bbb >= 0.5) and (caco2 >= CACO2_HIGH) and (free_frac >= FREE_FRACTION_FLOOR)
    call = "favourable" if favourable else "limited"
    return {"bbb_prob": round(bbb, 2), "caco2_logPapp": round(caco2, 2),
            ...
            "pgp_inhibition_prob": round(pgp_inhib, 2),
