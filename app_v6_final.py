...
from v6_tabs import render_v6_tabs
from pubchem_client import fetch_pubchem, fetch_chembl, fetch_kegg_pathways

# Real, validated 93-feature v5 ensemble for unknown-compound inference
# (replaces the weak session-fit RandomForest). Guarded so the app still runs
# if the model artifacts/engine are unavailable.
try:
    from ml_v5_engine import predict_compound as _v5_predict_compound
    _V5_ENGINE_OK = True
    _V5_ENGINE_ERR = ""
except Exception as _v5e:
    _V5_ENGINE_OK = False
    _V5_ENGINE_ERR = str(_v5e)

# Scientifically-grounded druggability scoring (RDKit; QED + Ro5 + Veber + CNS-MPO).
try:
    from BS_druggability import compute_druggability as _compute_druggability
    _DRUGGABILITY_OK = True
except Exception as _drge:
    _DRUGGABILITY_OK = False
    _DRUGGABILITY_ERR = str(_drge)
    def _compute_druggability(_smiles):   # graceful no-op fallback
        return {"ok": False}

# Genuine, scaffold-CV-validated antioxidant predictor (the ONE endpoint that is
# truly structure-predictable; see BS_MODEL_CARD.md section 10). The other six
# dimensions are NOT predicted for novel compounds because they are not
# structure-predictable (validated R2 <= 0).
try:
    from BS_predict import predict_antioxidant as _predict_antioxidant, antioxidant_available
    _GENUINE_OK = antioxidant_available()
except Exception as _ge:
    _GENUINE_OK = False
    def _predict_antioxidant(_s):
        return None
try:
...
@st.cache_data
def load_compounds():
    with open("BS_compounds_full.json", "r") as f:
        curated = json.load(f)
    merged = dict(curated)
    if os.path.exists("BS_compounds_ml_merged.json"):
        with open("BS_compounds_ml_merged.json", "r") as f:
            ml_raw = json.load(f)
        ml_raw.pop("_ml_metadata", None)
        for name, entry in ml_raw.items():
            if name not in merged:
                merged[name] = entry
    # Drop metadata/sentinel keys (e.g. "_note", "_metadata") so they never
    # surface as fake compounds in the dropdown.
...
def build_ml_predictor():
    ...
    try:
        ...
        with open("BS_compounds_full.json") as f:
            curated = json.load(f)
        X_rows, y_rows = [], []
...
def predict_unknown_via_ml(compound_name: str) -> dict | None:
    ...
    if model is not None and scaler is not None:
        try:
            ...
            scores = {col: round(float(np.clip(v, 1.0, 100.0)), 1)
    ...
    nps_final = v3_nps if v3_nps is not None else round(
        min(100.0, float(np.mean([scores.get(c, 50.0) for c in _ML_SCORE_COLS]))), 1)
...
def make_radar(data):
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        showlegend=True, hoverinfo='skip', name='Reference (50)'
    ))
    fig.add_trace(go.Scatterpolar(
        ...
        hovertemplate='<b>%{customdata}</b><br>Score: <b>%{r}/100</b><extra></extra>'
    ))
...
def generate_assessment(compound_name, data, score, enzyme_entry=None):
    ...
    top_dim_labels = [v[0] for _, v in top_dims if v[1] >= 50]
...
def render_druggability_panel(data):
    """
    Scientifically-grounded druggability panel computed live from SMILES via
    RDKit: QED (Bickerton 2012) + Lipinski Ro5 + Veber + CNS-MPO (Wager 2010),
    CNS-weighted because a brain-health compound must reach the CNS to matter.
    Silently no-ops when RDKit or a SMILES string is unavailable.
    """
    smi = data.get("smiles")
    if not (_DRUGGABILITY_OK and smi):
        return
    d = _compute_druggability(smi)
    if not d.get("ok"):
        return
    score = d["druggability"]
    fg, bg = (("#15803D", "#DCFCE7") if score >= 70 else
              ("#B45309", "#FEF3C7") if score >= 50 else ("#B91C1C", "#FEE2E2"))
    with st.container(border=True):
        st.markdown(
            '<p style="font-size:1.05rem;font-weight:700;color:#0D2137;margin:0 0 8px 0;">'
            'Druggability '
            '<span style="font-weight:500;font-size:0.8rem;color:#64748B;">'
            '(CNS-weighted: QED + Ro5 + Veber + CNS-MPO)</span></p>',
            unsafe_allow_html=True,
        )
        top = st.columns([1.4, 1, 1, 1, 1])
        top[0].markdown(
            f'<div style="background:{bg};border-radius:10px;padding:10px 6px;text-align:center;">'
            f'<div style="font-size:1.7rem;font-weight:800;color:{fg};line-height:1;">{score:.0f}'
            f'<span style="font-size:0.8rem;font-weight:600;">/100</span></div>'
            f'<div style="font-size:0.66rem;font-weight:700;color:{fg};margin-top:3px;">'
            f'{d["category"]}</div></div>',
            unsafe_allow_html=True,
        )
        top[1].metric("QED", f"{d['qed']:.2f}")
        top[2].metric("CNS-MPO", f"{d['cns_mpo']:.1f}/6")
        top[3].metric("Ro5 viol.", d["ro5_violations"])
        top[4].metric("Veber", "Pass" if d["veber_pass"] else "Fail")
        if d.get("pains_alerts"):
            st.caption(f"PAINS structural alerts: {d['pains_alerts']} "
                       f"(developability liability — applied as a small penalty).")
        st.caption(d["rationale"])
        st.caption("Druggability ≠ neuroprotection: it estimates developability "
                   "and CNS penetration, not efficacy. Method: QED (Bickerton 2012), "
                   "Lipinski Ro5 (2001), Veber (2002), CNS-MPO (Wager 2010).")


def render_genuine_prediction(compound_name, smiles, props=None):
    """
    Scientifically authentic view for a NOVEL compound (not in the curated DB):
    show ONLY validated outputs. The genuine antioxidant model (scaffold-CV
    R2=0.267 [0.182,0.346]) + the deterministic druggability score + factual
    physicochemistry. The other six neuroprotection dimensions are NOT shown,
    because they are not reliably predictable from structure (validated R2<=0).
    """
    props = props or {}
    st.markdown(
        f'<div style="background:#F0F9FF;border:1.5px solid #7DD3FC;border-radius:8px;'
        f'padding:10px 16px;margin:6px 0 12px 0;">'
        f'<b>{compound_name}</b> is not in the curated database. Below are only the '
        f'outputs that are <b>scientifically validated for novel compounds</b> — a '
        f'genuine antioxidant prediction and a deterministic druggability assessment. '
        f'No unvalidated multi-dimension "profile" is fabricated.</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.container(border=True):
        st.markdown(
            ...
            '<span style="font-weight:500;font-size:0.8rem;color:#64748B;">'
            '(genuine ML prediction)</span></p>', unsafe_allow_html=True)
        ap = _predict_antioxidant(smiles) if _GENUINE_OK else None
        if ap:
            v = ap["antioxidant"]; band = ap["uncertainty_band"]
            fg, bg = (("#15803D", "#DCFCE7") if v >= 67 else
                      ("#B45309", "#FEF3C7") if v >= 34 else ("#B91C1C", "#FEE2E2"))
            c1, c2 = st.columns([1, 2.4])
            c1.markdown(
                f'<div style="background:{bg};border-radius:10px;padding:12px 6px;text-align:center;">'
                f'<div style="font-size:1.8rem;font-weight:800;color:{fg};line-height:1;">{v:.0f}'
                f'<span style="font-size:0.8rem;font-weight:600;">/100</span></div>'
                f'<div style="font-size:0.64rem;font-weight:700;color:{fg};margin-top:3px;">'
                f'~{band[0]:.0f}–{band[1]:.0f} (±MAE)</div></div>', unsafe_allow_html=True)
            c2.caption(
                f"Genuine, leak-free prediction — scaffold-CV R²={ap['scaffold_cv_r2']} "
                f"(95% CI {ap['ci95'][0]}–{ap['ci95'][1]}), Spearman ρ={ap['spearman']}. "
                f"Method: {ap['method']}; trained on human/literature labels, structure-only "
                f"features. Modest but real: continuous assay endpoints (ORAC/DPPH) would improve it.")
        else:
            st.info("Antioxidant model unavailable, or structure could not be parsed.")

    st.warning(
        "**Only antioxidant is reliably predictable from chemical structure.** The other "
        "six neuroprotection dimensions (anti-inflammatory, mitochondrial support, "
        "aggregation modulation, cognitive enhancement, neurogenesis, synaptic plasticity) "
        "are **not structure-predictable** (validated R²≤0) and are therefore **not estimated** "
        "for novel compounds — they exist only for compounds with curated literature evidence. "
        "See BS_MODEL_CARD.md.")

    # Deterministic, validated druggability (works from SMILES alone)
    render_druggability_panel({"smiles": smiles})

    # Factual physicochemistry (measured/computed, not predicted)
    if props and not props.get("error"):
        with st.container(border=True):
            st.markdown('<p style="font-size:1.0rem;font-weight:700;color:#0D2137;margin:0 0 6px 0;">'
                        'Physicochemical properties <span style="font-weight:500;font-size:0.8rem;'
                        'color:#64748B;">(PubChem / RDKit — factual)</span></p>', unsafe_allow_html=True)
            cols = st.columns(5)
            cols[0].metric("Formula", str(props.get("formula", "N/A")))
            cols[1].metric("MW", f"{props.get('mw', 'N/A')}")
            cols[2].metric("XLogP", f"{props.get('xlogp', 'N/A')}")
            cols[3].metric("TPSA", f"{props.get('tpsa', 'N/A')}")
            cols[4].metric("PubChem CID", str(props.get("cid", "N/A")))


def render_brain_profile_panel(compound_name, smiles, props=None):
    """
    GENUINE multi-endpoint brain-effect profile for a novel compound, from models
    trained on MEASURED data (B3DB + ChEMBL pChEMBL), each with honest scaffold-CV
    AUROC and an applicability-domain flag. Falls back to the antioxidant+druggability
    view if the endpoint models are unavailable.
    """
    props = props or {}
    try:
        from BS_brain_predict import predict_brain_profile
        prof = predict_brain_profile(smiles)
    except Exception:
        prof = {"ok": False, "endpoints": []}

    if not prof.get("ok") or not prof.get("endpoints"):
        return render_genuine_prediction(compound_name, smiles, props)

    st.markdown(
        f'<div style="background:#F0F9FF;border:1.5px solid #7DD3FC;border-radius:8px;'
        f'padding:10px 16px;margin:6px 0 12px 0;"><b>{compound_name}</b> — genuine '
        f'brain-effect profile predicted from <b>measured bioactivity data</b> '
        f'(ChEMBL pChEMBL + B3DB). Every endpoint below is an independently validated '
        f'model; values are probabilities with the model’s honest scaffold-CV AUROC '
        f'and an applicability-domain flag.</div>', unsafe_allow_html=True)

    eps = prof["endpoints"]
    bbb = next((e for e in eps if e["endpoint"] == "BBB"), None)
    targets = [e for e in eps if e["endpoint"] != "BBB"]

    # --- BBB gate ---
    if bbb:
        pen = bbb["call"] == "Penetrant"; p = bbb["probability"] * 100
        fg, bg = ("#15803D", "#DCFCE7") if pen else ("#B91C1C", "#FEE2E2")
        dom = "" if bbb["in_domain"] else " · ⚠ novel scaffold (low confidence)"
        with st.container(border=True):
            c1, c2 = st.columns([1, 2.6])
            c1.markdown(
                f'<div style="background:{bg};border-radius:10px;padding:12px 6px;text-align:center;">'
                f'<div style="font-size:1.4rem;font-weight:800;color:{fg};line-height:1.1;">{bbb["call"]}</div>'
                f'<div style="font-size:0.7rem;color:{fg};font-weight:700;">P={p:.0f}%</div></div>',
                unsafe_allow_html=True)
            c2.markdown(f"**Blood-brain barrier penetration** — does it reach the brain?  \n"
                        f"Model AUROC **{bbb['auroc']}** (B3DB, n={bbb['n_train']}); "
                        f"applicability T={bbb['applicability_tanimoto']}{dom}")

    # --- disease/mechanism targets ---
    if targets:
        rows = ""
        for e in targets:
            active = e["call"].startswith("Active")
            col = "#1B6B45" if active else "#9B2335"
            p = e["probability"] * 100
            dom = "in-domain" if e["in_domain"] else "⚠ extrapolation"
            rows += (f'<tr><td style="font-weight:600;">{e["endpoint"].replace("_","-")}</td>'
                     f'<td style="font-size:0.82rem;color:#475569;">{e["meaning"]}</td>'
                     f'<td style="text-align:center;font-weight:700;color:{col};">{e["call"]}</td>'
                     f'<td style="text-align:right;">{p:.0f}%</td>'
                     f'<td style="text-align:right;color:#64748B;">{e["auroc"]}</td>'
                     f'<td style="text-align:right;font-size:0.78rem;color:#64748B;">{dom}</td></tr>')
        with st.container(border=True):
            st.markdown(
                '<p style="font-size:1.0rem;font-weight:700;color:#0D2137;margin:0 0 6px 0;">'
                'Disease-relevant target activity <span style="font-weight:500;font-size:0.8rem;'
                'color:#64748B;">(measured ChEMBL QSAR)</span></p>'
                f'<table><thead><tr><th>Target</th><th>Relevance</th><th style="text-align:center;">Call</th>'
                f'<th style="text-align:right;">P(active)</th><th style="text-align:right;">AUROC</th>'
                f'<th style="text-align:right;">domain</th></tr></thead><tbody>{rows}</tbody></table>',
                unsafe_allow_html=True)

    # --- antioxidant (genuine) ---
    ant = prof.get("antioxidant")
    if ant:
        v = ant["antioxidant"]
        with st.container(border=True):
            st.markdown(f'**Antioxidant capacity (genuine ML):** **{v:.0f}/100** '
                        f'(scaffold-CV R²={ant["scaffold_cv_r2"]}, CI {ant["ci95"][0]}–{ant["ci95"][1]}).')

    # --- deterministic druggability + facts ---
    render_druggability_panel({"smiles": smiles})
    if props and not props.get("error"):
        with st.container(border=True):
            cols = st.columns(5)
            ...
            cols[3].metric("TPSA", f"{props.get('tpsa', 'N/A')}")
            cols[4].metric("PubChem CID", str(props.get("cid", "N/A")))
    st.caption("All endpoint models trained on measured public data (ChEMBL pChEMBL ≥6 active / <5 "
               "inactive; B3DB for BBB), validated by scaffold-split CV. Predictions outside the "
               "applicability domain (novel scaffolds) are flagged and should be treated as low-confidence "
               "hypotheses. Research-use only — not medical advice. See BS_MODEL_CARD.md.")


def render_brain_profile_v2(compound_name, smiles, props=None):
    """Innovation dashboard for a novel compound: BBB-gated per-disease engagement,
    measured-data target activity with nearest-analog EVIDENCE, a SAFETY (hERG) axis,
    calibrated probabilities, antioxidant + druggability, and a transparent verdict."""
    props = props or {}
    try:
        from BS_brain_predict import predict_brain_profile
        prof = predict_brain_profile(smiles)
    ...
    if not prof.get("ok") or not prof.get("endpoints"):
        return render_genuine_prediction(compound_name, smiles, props)
    eps = {e["endpoint"]: e for e in prof["endpoints"]}

    st.markdown(
        f'<div style="background:#F0F9FF;border:1.5px solid #7DD3FC;border-radius:8px;'
        f'padding:10px 16px;margin:6px 0 10px 0;"><b>{compound_name}</b> — brain-effect '
        f'profile from models trained on <b>measured bioactivity</b> (ChEMBL + B3DB), '
        f'with calibrated probabilities, BBB-gated disease scores, a safety axis, and '
        f'real measured-analog evidence behind every call.</div>', unsafe_allow_html=True)

    if prof.get("verdict"):
        st.markdown(f"**Verdict:** {prof['verdict']}")

    # --- BBB-gated per-disease engagement (headline) ---
    dis = prof.get("diseases", [])
    if dis:
        st.markdown('<p style="font-weight:700;color:#0D2137;margin:6px 0 4px 0;">'
                    'Disease-relevant CNS engagement '
                    '<span style="font-weight:500;font-size:0.8rem;color:#64748B;">'
                    '(BBB-gated: P(target)×P(brain entry))</span></p>', unsafe_allow_html=True)
        cols = st.columns(max(1, min(4, len(dis))))
        for col, d in zip(cols, dis[:4]):
            sc = d["score"] * 100
            fg, bg = (("#15803D", "#DCFCE7") if sc >= 50 else
                      ("#B45309", "#FEF3C7") if sc >= 30 else ("#7F1D1D", "#FEE2E2"))
            col.markdown(
                f'<div style="background:{bg};border-radius:10px;padding:10px 6px;text-align:center;">'
                f'<div style="font-size:0.72rem;font-weight:700;color:{fg};">{d["disease"]}</div>'
                f'<div style="font-size:1.5rem;font-weight:800;color:{fg};">{sc:.0f}%</div>'
                f'<div style="font-size:0.62rem;color:{fg};">via {d["driver"].replace("_","-")}</div></div>',
                unsafe_allow_html=True)
    ...
    bbb = eps.get("BBB")
    if bbb:
        pen = bbb["call"] == "Penetrant"
        cc[0].markdown(f"**Brain penetration (BBB):** "
                       f"{'🟢 ' if pen else '🔴 '}**{bbb['call']}** "
                       f"(P={bbb['probability']*100:.0f}%, AUROC {bbb['auroc']})")
    saf = prof.get("safety")
    if saf:
        rc = {"High": "🔴", "Moderate": "🟠", "Low": "🟢"}.get(saf["risk"], "")
        cc[1].markdown(f"**Safety — hERG cardiotox:** {rc} **{saf['risk']} risk** "
                       f"(P(blocker)={saf['herg_p']*100:.0f}%)")

    # --- target activity table (measured QSAR) ---
    targets = [e for e in prof["endpoints"] if e["kind"] == "target"]
    if targets:
        rows = ""
        for e in targets:
            active = e["call"] == "Active"
            col = "#1B6B45" if active else "#9B2335"
            c = e.get("conformal") or {}
            cset = c.get("set") or []
            if cset == ["active"]:
                conf = "active 90%"
            elif cset == ["inactive"]:
                conf = "inactive 90%"
            elif set(cset) == {"active", "inactive"}:
                conf = "uncertain"
            else:
                conf = "out-of-dist."
            if not e["in_domain"]:
                conf += " ⚠"
            rows += (f'<tr><td style="font-weight:600;">{e["endpoint"].replace("_","-")}</td>'
                     f'<td style="font-size:0.8rem;color:#475569;">{e["meaning"]}</td>'
                     f'<td style="text-align:center;font-weight:700;color:{col};">{e["call"]}</td>'
                     f'<td style="text-align:right;">{e["probability"]*100:.0f}%</td>'
                     f'<td style="text-align:right;color:#64748B;">{e["auroc"]}</td>'
                     f'<td style="text-align:right;font-size:0.76rem;color:#64748B;">{conf}</td></tr>')
        st.markdown(
            '<p style="font-weight:700;color:#0D2137;margin:8px 0 4px 0;">Target activity '
            '<span style="font-weight:500;font-size:0.8rem;color:#64748B;">(measured ChEMBL QSAR, '
            'calibrated)</span></p>'
            f'<table><thead><tr><th>Target</th><th>Relevance</th><th style="text-align:center;">Call</th>'
            f'<th style="text-align:right;">P</th><th style="text-align:right;">AUROC</th>'
            f'<th style="text-align:right;">conformal</th></tr></thead><tbody>{rows}</tbody></table>',
            unsafe_allow_html=True)

    # --- EVIDENCE: nearest measured analogs (the trust layer) ---
    with st.expander("🔬 Evidence — nearest real measured compounds behind each call"):
        for e in prof["endpoints"]:
            if not e.get("evidence"):
                continue
            line = f"**{e['endpoint'].replace('_','-')}** ({e['meaning']}): "
            chips = []
            for a in e["evidence"]:
                pv = f", pChEMBL {a['pchembl']:.1f}" if a.get("pchembl") else ""
                chips.append(f"T={a['similarity']} · {a['measured']}{pv}")
            st.markdown(line + " | ".join(chips))
        st.caption("Each call is grounded in the most structurally similar compounds that have "
                   "REAL measured activity. High similarity (T) = strong evidence; low T = extrapolation.")

    # --- receptor binding potency (predicted pKi; measured-data regression) ---
    rp = prof.get("receptor_potency") or []
    if rp:
        rrows = "".join(
            f'<tr><td style="font-weight:600;">{r["target"].replace("_","-")}</td>'
            f'<td style="font-size:0.8rem;color:#475569;">{r["meaning"]}</td>'
            f'<td style="text-align:right;font-weight:700;">{r["predicted_pKi"]}</td>'
            f'<td style="text-align:right;color:#64748B;">R²={r["scaffold_cv_r2"]} / ρ={r["spearman"]}</td></tr>'
            for r in rp)
        st.markdown(
            '<p style="font-weight:700;color:#0D2137;margin:8px 0 4px 0;">Receptor binding potency '
            '<span style="font-weight:500;font-size:0.8rem;color:#64748B;">(predicted pKi — measured-data '
            'regression; ranking-grade)</span></p>'
            f'<table><thead><tr><th>Receptor</th><th>Relevance</th><th style="text-align:right;">pred. pKi</th>'
            f'<th style="text-align:right;">scaffold-CV</th></tr></thead><tbody>{rrows}</tbody></table>',
            unsafe_allow_html=True)
        st.caption("Predicted binding potency (−log₁₀ Ki/IC₅₀). Ranking-grade (Spearman ≈0.57–0.71); "
                   "temporal generalisation is weak — use for prioritisation, not absolute potency.")

    # --- antioxidant + druggability + facts ---
    ant = prof.get("antioxidant")
    if ant:
        extra = f", measured DPPH pIC₅₀={ant['pic50']}" if ant.get("pic50") is not None else ""
        st.markdown(f"**Antioxidant (measured-data model):** **{ant['antioxidant']:.0f}/100**{extra} "
                    f"(scaffold-CV R²={ant['scaffold_cv_r2']}, ρ={ant.get('spearman')}).")

    # --- clinical / translational precedent (real ChEMBL clinical-phase data) ---
    ce = prof.get("clinical_evidence") or []
    if ce:
        chips = " | ".join(
            f"{c['name']} (phase {c['max_phase']}, {c['disease']}, T={c['similarity']})" for c in ce)
        st.markdown('<p style="font-weight:700;color:#0D2137;margin:8px 0 2px 0;">Clinical / translational '
                    'precedent <span style="font-weight:500;font-size:0.8rem;color:#64748B;">'
                    '(nearest CNS compounds that reached human trials — ChEMBL clinical phase)</span></p>',
                    unsafe_allow_html=True)
        st.caption("Engagement ≠ efficacy. This shows whether the molecule resembles compounds with "
                   "real clinical precedent (not a prediction of clinical success): " + chips)
    ...
    if props and not props.get("error"):
        ...
        cols[3].metric("TPSA", f"{props.get('tpsa', 'N/A')}")
        cols[4].metric("PubChem CID", str(props.get("cid", "N/A")))
    st.caption("All endpoints trained on measured public data (ChEMBL pChEMBL; B3DB for BBB), "
               "scaffold-CV validated, isotonic-calibrated probabilities. Out-of-domain (novel-scaffold) "
               "calls are flagged and are low-confidence. Predicts target engagement, not clinical "
               "efficacy. Research-use only — not medical advice. See BS_MODEL_CARD.md.")


def render_report(compound_name, data):
    # Canonicalise dimension scores to the 0-100 scale that scorer.py and the
    # 535-compound BS database use. DB and live ML-predicted entries are already
    # 0-100; only the hard-coded class-estimated templates remain on a legacy
    # 0-10 scale, so scale those up here — on a copy, never mutating the source.
    _DIM_KEYS = ("antioxidant", "anti_inflammatory", "mitochondrial_support",
                 "aggregation_modulation", "cognitive_enhancement",
                 "neurogenesis", "synaptic_plasticity")
    if data.get("is_estimated"):
        ...
        for _d in _DIM_KEYS:
            if _d in data:
                data[_d] = round(float(data[_d] or 0) * 10, 1)
    ...
    if _is_est:
        ...
        if _live_ml:
            _ml_body = (
                ...
                f'. A 4-model ensemble (validated under leak-free scaffold-split CV, NPS R2~0.57) '
                f'<b>Caveat: relies largely on known disease-target annotations, not structure; '
                f'unreliable for novel compounds (see BS_MODEL_CARD.md). The Druggability panel '
                f'below is a deterministic non-ML metric.</b> Estimated profile: '
            ...
            _ml_body = (
                ...
                f'were estimated by a 4-model ensemble (validated under leak-free scaffold-split CV, NPS R2~0.57). '
                f'<b>Caveat: the model relies largely on known disease-target annotations, not chemical '
                f'structure, so it is unreliable for novel compounds (see BS_MODEL_CARD.md); the Druggability '
                f'panel below is a deterministic, structure-based, non-ML metric.</b> '
    ...
    with col_right:
        with st.container(border=True):
            ...
            st.markdown(
                f'<table><thead><tr><th>Dimension</th><th>Score</th><th style="text-align:right;">/100</th></tr>'
                f'</thead><tbody>{rows}</tbody></table>',
                unsafe_allow_html=True
            )

    # v6: Neurotransmitter / Brain Region / Dose-Response tabs
    _dim_scores_100 = {
        "antioxidant":            data.get("antioxidant", 0),
        "anti_inflammatory":      data.get("anti_inflammatory", 0),
        "mitochondrial_support":  data.get("mitochondrial_support", 0),
        "aggregation_modulation": data.get("aggregation_modulation", 0),
        "cognitive_enhancement":  data.get("cognitive_enhancement", 0),
        "neurogenesis":           data.get("neurogenesis", 0),
        "synaptic_plasticity":    data.get("synaptic_plasticity", 0),
    }
    # Build the entry dict expected by render_v6_tabs: the compound's data
    # (compound_type, mw, mechanisms, scores, ...) plus its display name.
    # Shallow copy so the original `data` dict is never mutated.
    entry = {**data, "name": compound_name}
    render_v6_tabs(entry, _dim_scores_100, patient_mode=False)

    # Scientifically-grounded druggability (developability + CNS penetration)
    render_druggability_panel(data)
...
<div class="site-header">
    <div class="header-logo-wrap">
        ...
        <div class="header-text-block">
            ...
            <div class="header-sub">Multi-endpoint prediction of small-molecule effects on the brain &mdash;
                trained on measured ChEMBL &amp; B3DB bioactivity data</div>
            <div class="header-tags">
                <span class="header-tag-pill">BBB penetration</span>
                <span class="header-tag-pill">AChE / BChE</span>
                <span class="header-tag-pill">BACE1</span>
                <span class="header-tag-pill">GSK-3&beta;</span>
                <span class="header-tag-pill">MAO-A / MAO-B</span>
                <span class="header-tag-pill">hERG safety</span>
                <span class="header-tag-pill">Antioxidant</span>
                <span class="header-tag-pill">Druggability / CNS-MPO</span>
                <span class="header-tag-pill">Calibrated &amp; conformal</span>
                <span class="header-tag-pill">Evidence-grounded</span>
            </div>
        </div>
        <div class="header-divider"></div>
        {_sssihl_logo_html}
...
st.markdown(
    '<div style="background:#FFF8E6;border:1px solid #F0C674;border-left:5px solid #F0A500;'
    'border-radius:8px;padding:10px 16px;margin:4px 0 14px 0;font-size:0.85rem;color:#7A5B00;">'
    '<b>Research preview &mdash; pending peer review.</b> BrainSafe AI is a computational '
    'decision-support tool for research prioritisation and hypothesis generation. It predicts '
    '<i>molecular target engagement and physicochemical properties</i>, not clinical efficacy, and '
    'has not undergone wet-lab or clinical validation. Not for medical, diagnostic, or treatment '
    'decisions.</div>',
    unsafe_allow_html=True,
)
...
with tab_search:
    ...
    with st.container(border=True):
        st.markdown(
            ...
            'Select a curated compound from the dropdown, or type any compound name below. '
            'Unknown compounds are resolved to a structure via PubChem and profiled across all '
            'measured-data endpoints (BBB, CNS targets, hERG safety, antioxidant, druggability), '
            'each with calibrated probability, conformal confidence and nearest measured-analogue '
            'evidence. Common abbreviations (EGCG, NAC, CoQ10, etc.) are supported.</p>',
            unsafe_allow_html=True
        )
    ...
    if search_btn and selected:
        ...
        if matches:
            ...
            # Scientific-authenticity policy (see BS_MODEL_CARD.md): for a novel
            # compound we present ONLY validated outputs. The previous circular
            # 93-feature "7-dimension profile" and the unvalidated name-based
            # class estimate have been removed; we resolve a real structure and
            # show the genuine antioxidant model + deterministic druggability.
            with st.spinner("Resolving chemical structure from PubChem..."):
                _pc = fetch_pubchem(free_text.strip())
            _smi = "" if (not _pc or _pc.get("error")) else str(_pc.get("smiles", "") or "")
            if _smi and _smi.strip().lower() not in ("", "n/a", "nan"):
                st.success(
                    f"**'{free_text}'** resolved to a chemical structure "
                    f"(PubChem CID {_pc.get('cid', '?')}). Showing only scientifically "
                    f"validated outputs for this novel compound."
                )
                render_brain_profile_v2(free_text.strip().title(), _smi, _pc)
            else:
                st.warning(
                    f"**'{free_text}'** could not be resolved to a chemical structure via PubChem "
                    f"(offline, misspelled, or not catalogued). A structure (SMILES) is required for a "
                    f"scientifically valid assessment; unvalidated name-based class 'estimates' have been "
                    f"removed because they are not predictive. Try a different name or a known synonym."
                )
...
with tab_about:
    ...
    with st.container(border=True):
        ...
        BrainSafe AI estimates, from chemical structure alone, a compound's likely profile of effects
        relevant to the brain. For any drug or natural product (e.g. a flavonoid) it predicts:
        ...
        - **Disease-relevant target activity** — AChE & BChE (Alzheimer's/cognition), BACE1 (amyloid),
          GSK-3β (tau/neuroprotection), MAO-B (Parkinson's), MAO-A (mood), and receptor potencies
          (D2, A2A, 5-HT2A, SERT).
        - **Safety** — hERG cardiotoxicity liability.
        - **Antioxidant** capacity (measured DPPH model) and **druggability / CNS-MPO**.
        - **BBB-gated per-disease engagement**, a **conformal confidence** statement, and the
          **nearest real measured compounds** behind each prediction.
        """)
    ...
    with st.container(border=True):
        ...
        Every endpoint is trained on **measured experimental data** — ChEMBL bioassay measurements
        (pChEMBL) and the B3DB blood–brain-barrier database — totalling **64,474 measured
        compound–endpoint records**. Molecules are represented by ECFP-4 fingerprints and RDKit
        physicochemical descriptors; predictions use an ensemble of random forest, extremely
        randomised trees and histogram gradient boosting, with **isotonic calibration** and
        **Mondrian conformal prediction**.

        Models were validated under four regimes of increasing stringency — random, scaffold,
        leave-cluster-out and **temporal (future-compound)** splits. On like-for-like random splits
        the classifiers reach AUROC 0.94–0.98; under scaffold/cluster splits 0.87–0.95; and under
        temporal splits 0.61–0.92. Full methods, figures and per-endpoint metrics are reported in
        the accompanying manuscript and model card.
        """)
    ...
    with st.container(border=True):
        ...
        BrainSafe AI predicts **molecular target engagement and physicochemical properties, not
        clinical efficacy**, and does not distinguish agonism from antagonism. Predictions outside the
        models' applicability domain (novel chemotypes) are flagged as low-confidence. The clinical
        layer reports structural similarity to compounds with documented clinical phases — it is
        **clinical precedent, not an efficacy prediction**. The tool is intended for research
        prioritisation and hypothesis generation and is **pending peer review**; it is **not** for
        medical, diagnostic, or treatment use.
        """)
    ...
    with st.container(border=True):
        ...
        <p style="font-size:0.85rem;color:#475569;line-height:1.75;margin:0;">
          Target bioactivities (measured pChEMBL values: IC50, Ki, Kd, EC50)
          were obtained from <strong>ChEMBL</strong> (EMBL-EBI); blood–brain-barrier permeability
          from the <strong>B3DB</strong> database; antioxidant activity from <strong>ChEMBL DPPH
          radical-scavenging assays</strong>; and clinical-phase annotations from <strong>ChEMBL
          ATC-classified compounds</strong>. Structures for user-entered compounds are resolved via
          <strong>PubChem</strong>. Molecular descriptors and fingerprints were computed with
          <strong>RDKit</strong>; models were built with <strong>scikit-learn</strong>. All models
          are trained exclusively on measured experimental data; full provenance, methods and
          validation are reported in the accompanying manuscript and model card. Predictions should
          be independently verified before any research use.
        </p>
        """, unsafe_allow_html=True)